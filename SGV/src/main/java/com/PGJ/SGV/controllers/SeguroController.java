...
public class SeguroController {
	...
	public String listar(@RequestParam(name = "page", defaultValue = "0") int page, Model model,
		...
		if (user.equals("ROLE_USER")) {
			...
			// segurosArea =
			// seguroService.FindSeguroArea(usus.getAdscripcion().getId_adscripcion());
			Page<Seguro> SeguroAreaPage = seguroService.FindSeguroAreaPage(usus.getAdscripcion().getId_adscripcion(),
					pageRequest);
			PageRender<Seguro> SeguroRenderArea = new PageRender<>("/Seguros", SeguroAreaPage);
			if (seguroService.totalSeguros() >= 7) {
	...
	public String Buscartabla(@RequestParam(name = "page", defaultValue = "0") int page, Authentication authentication,
		...
		if (!elemento.isBlank()) {
			...
			if (user.equals("ROLE_USER")) {
				...
				Page<Seguro> segurospage = seguroService.FindSegElemAreaPage(usus.getAdscripcion().getId_adscripcion(),
						"%" + elemento + "%", pageRequest);
