...
public interface IMantenimientoDao extends PagingAndSortingRepository<Mantenimiento, Long>{
	...
	@Query("select count(m) from Mantenimiento m inner join Vehiculo v on m.vehiculo.placa = v.placa inner join Adscripcion a on v.adscripcion.id_adscripcion=a.id_adscripcion where a.id_adscripcion = ?1")
